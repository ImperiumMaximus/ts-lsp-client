...
import { ClientCapabilities, DidCloseTextDocumentParams, DidOpenTextDocumentParams, DocumentSymbol, SymbolKind, Location, SignatureHelp, CompletionList } from "../src/models";
...
describe('LspClient', () => {
  ...
  it('sends a LSP textDocument/completion request', async () => {
    const mockWriteStream: WriteMemory = new WriteMemory();
    const mockReadStream: Readable = new Readable();
    const resCompletion: CompletionList = {
      isIncomplete: false,
      itemDefaults: {
        commitCharacters: [],
        editRange: {
    ...
    const completionRespose: JSONRPCResponse = {
    ...
    expect(response).toEqual(resCompletion);
  });
});
